...
<h2 id=12>Qt and c++混合编程</h2>

- Qt Quick技术的引入使我们快速构建流畅的用户界面，且具有动画、各种绚丽效果，但其不是万能的。
- 这个技术也有很多局限性，原来的部分qt技术，比如低阶的网络编程如QTcpSocket、多线程，又如XML文档处理类库QXmlStreamReader、QXmlStreamWriter，再如再如文件操作QFile、QTextStream等，在QML中要么不可用，要么用i起来不方便。
- 所以基于此，我们会混合使用QML和2c++，使用QML构建界面，使用C++实现非界面的业务逻辑和复杂运算。

<h3>在QML中使用C++类和对象</h3>

- 如果想在QML中访问C++对象，必须要找到一种途径在两个运行环境之间建立沟通的桥梁。
- 这里有两种在QML中使用C++对象的方式：
...
- 2、使用Q_OBJECT宏
- 作用：
- 1、让一个类能够进入QT强大的元对象系统(meta-object system)中，只有使用了元对象系统，一个类的某些方法或属性才能通过字符串的形式的名字调用，才具有了在QML访问的基本条件
- 什么特征的属性或方法才可以被QML访问？
- 1、信号和槽
- 只要是信号或者槽，都可以在QML中访问，你可以用c++对象的信号连接到QML中定义的方法，也可以把QML对象的信号连接到C++对象的槽上，还可以直接调用c++的信号和槽，也是最好的途径。
- 2、Q_INVOKABLE 宏
- 在定义一个成员函数时使用Q_INVOKABLE宏修饰好似，就可以使该方法被对象系统调用，必须放在返回值的前面。一旦使用了这个宏，在QML中就可以用${Object}.${method}来访问。
- 3、Q_ENUMS宏
- 如果你需要导出类定义了且想在QML使用的枚举型，可以使用Q_ENUMS宏将枚举型注册到元对象系统中。
- 4、Q_PROPERTY宏
- Q_PROPERTY宏用来定义可通过元对象系统访问的属性，通过它定义的属性，可以在QML访问、修改，也可以变化时发出特定的信号。
- 格式：Q_PROPERTY(type name (READ getFunc \[WRITE setFunc] | MEMBER memberName \[(READ getFunc \[WRITE setFunc])] ) \[RESET resetFunc] \[NOTIFY notifySignal] \[REVISION int] ...)
